...
describe("Note content - Content Only", () => {
    ...
    beforeAll(async () => {
        fileContents = await osf.loadFileContent(newLocal);
    ...
    it("Last line content", () => {
        const noteContent = doc.noteContent(content[0], content.slice(1), true);
        expect(osf.lastLine(noteContent)).toBe("- How to [[Working with multiple notes|open multiple files side by side]]");
    });
...
describe("Note content - Content Only - Normalize header levels", () => {
    ...
    beforeAll(async () => {
        fileContents = await osf.loadFileContent(newLocal);
        content = osf.toArray(fileContents, 42, 51);
    ...
    it("First line content", () => {
        const noteContent = doc.noteContent(content[0], content.slice(1), true);
        expect(osf.firstLine(noteContent)).toBe("# I have questions.");
    });

    it("Header 3 content", () => {
        const noteContent = doc.noteContent(content[0], content.slice(1), true);
        expect(osf.toArray(noteContent)[4]).toBe("## Header 3");
    });

    it("Last line content", () => {
        const noteContent = doc.noteContent(content[0], content.slice(1), true);
        expect(osf.lastLine(noteContent)).toBe("This is for testing normalizing header levels.");
    });
